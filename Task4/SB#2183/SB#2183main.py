...
def aruco_detection_image(ip_image):
    ...
    img1,aruco_centre=mark_Aruco(ip_image1,id1)
...
def coin_detection_image(ip_image):
    ...
    #******************
        ...
        if (not bool1 or not bool2 or not bool3):
            ...
            return output_image,x_green,y_green,x_red,y_red,centre_x,centre_y
...
def main():
    ...
    while(ret):
                ...
                break
    angle_aruco_r=math.acos((((x_red-centre_x)*(aruco_centre[0]-centre_x))+((y_red-centre_y)*(aruco_centre[1]-centre_y)))/(math.sqrt((centre_x-aruco_centre[0])**2+(aruco_centre[1]-centre_y)**2)*math.sqrt(((x_red-centre_x)**2+(y_red-centre_y)**2))))*(180/3.14)
    angle_aruco_g=math.acos((((x_green-centre_x)*(aruco_centre[0]-centre_x))+((y_green-centre_y)*(aruco_centre[1]-centre_y)))/ (math.sqrt((centre_x-aruco_centre[0])**2+ (aruco_centre[1]-centre_y)**2)*math.sqrt(((x_green-centre_x)**2+(y_green-centre_y)**2))))*(180/3.14)
